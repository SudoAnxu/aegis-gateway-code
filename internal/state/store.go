...
func (s *Store) checkRefundLocked(id string) error {status,exists:=s.transactions[id];if !exists{if mutation.AllowRefundWithoutCreate(){return nil};return fmt.Errorf("state_precondition")};if !mutation.SkipRefundReplay()&&(status==Refunded||status==Refunding){return fmt.Errorf("state_replay")};if status!=Created&&!mutation.AllowRefundWithoutCreate(){return fmt.Errorf("state_invalid_transition")};return nil}
