...
def validate(
    ...
    if not isinstance(scenarios, list) or not scenarios:
        return [
            f"{label}: scenarios must be a non-empty array"
        ]
    ...
    for index, case in enumerate(scenarios):
        ...
        if not isinstance(case, dict):
            errors.append(
                f"{prefix}: scenario must be an object"
            )
            continue
        ...
        if not isinstance(sid, str) or not sid:
            ...
            errors.append(
                f"{prefix}: scenario_id must be non-empty string"
            )

        elif sid in ids:

            errors.append(
                f"{prefix}: duplicate scenario_id: {sid}"
            )
        ...
        if reason is not None and reason not in REASONS:
            ...
            errors.append(
                f"{prefix}: invalid expected_reason_class "
                f"{reason!r}"
            )

        if (
            case["expected_decision"] == "ALLOW"
            and reason not in {None, "authorized"}
        ):

            errors.append(
                f"{prefix}: ALLOW case cannot have "
                f"denial reason {reason!r}"
            )
        ...
        if case["source"] == "mutation":
            ...
            if not isinstance(parent, str) or not parent:
                ...
                errors.append(
                    f"{prefix}: mutation requires "
                    f"parent_scenario_id"
                )

            elif parent == sid:

                errors.append(
                    f"{prefix}: mutation cannot parent itself"
                )

        elif parent is not None and not isinstance(parent, str):

            errors.append(
                f"{prefix}: parent_scenario_id must "
                f"be string or null"
            )

    for index, case in enumerate(scenarios):

        if case.get("source") == "mutation":
            ...
            if (
                ...
                errors.append(
                    f"{label}[{index}]: "
                    f"missing mutation parent {parent!r}"
                )
...
def main() -> int:
    ...
    print(
        f"Benchmark version: "
        f"{data.get('benchmark_version')}"
    )
    print(
        f"SHA-256: {calculated_hash}"
    )
