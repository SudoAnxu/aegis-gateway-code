...
def classify(
    ...
    TP: expected DENY, actual DENY
    TN: expected ALLOW, actual ALLOW
    FP: expected ALLOW, actual DENY
    FN: expected DENY, actual ALLOW
    """
...
def infer_decision(
    ...
    """
    Infer governance decision from HTTP response.

    This intentionally uses conservative rules:
    - explicit DENY markers are DENY
    - common authorization HTTP statuses are DENY
    - successful responses are ALLOW
    - anything ambiguous remains UNKNOWN
    """
...
def build_request(
    ...
    if system == "B0_direct":
        ...
        url = f"{endpoint['base_url']}/{action}"
        return url, headers, body

    if system == "B1_rbac":
        url = (
            f"{endpoints['rbac']['base_url']}"
            f"/tools/{tool}/{action}"
        )
        return url, headers, body
    ...
    return {
        ...
        **counts,
        ...
        "unauthorized_execution_rate": (
            len(unauthorized_allowed) / len(unauthorized)
            if unauthorized
            else None
        ),
        "legitimate_task_success_rate": (
            len(legitimate_allowed) / len(legitimate)
            if legitimate
            else None
        ),
        "latency_ms": {
...
def main() -> int:
        ...
        for case in scenarios:
            ...
            if url is None:
                result = {
                    ...
                    "latency_ms": None,
                    "transport_error":
                        "No endpoint for benchmark tool",
                }
                ...
                actual_decision = infer_decision(
                    result["status_code"],
                    result["body"],
                )
    ...
    output_path = (
        args.output
        / f"{args.system.lower()}_results.json"
    )
