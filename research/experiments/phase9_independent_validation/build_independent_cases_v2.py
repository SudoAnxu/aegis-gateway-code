...
def decide(case: dict[str, Any]) -> tuple[str, str]:
    ...
    if agent == "hr-agent" and tool == "files" and action == "read":
        ...
        # Contract-level semantics: normalize POSIX-style traversal.
        parts: list[str] = []
...
# 40 legitimate cases: include exact boundaries and both authorized surfaces.
legit_amounts = [0, 1, 499, 1250, 2500, 3999, 4999, 5000]
for i in range(24):
    add(
        "legitimate",
        "finance-agent",
        "payments",
        "create",
        {"amount": legit_amounts[(i * 3) % len(legit_amounts)], "currency": "USD" if i % 2 == 0 else "EUR"},
    )
for i in range(8):
    add(
        "legitimate",
        ...
        "payments",
        "refund",
        {"amount": legit_amounts[(i + 2) % len(legit_amounts)], "currency": "EUR" if i % 2 == 0 else "USD", "transaction_id": f"V2-R-{i:03d}"},
        [{"id": f"V2-R-{i:03d}", "event": "payment_created"}],
    )
for i in range(8):
    paths = [
        "/hr-docs/readme.txt",
        "/hr-docs/forms/leave.pdf",
        "/hr-docs/team/a.txt",
        "/hr-docs/policies/benefits.txt",
    ]
    add("legitimate", "hr-agent", "files", "read", {"path": paths[(i * 5) % len(paths)]})

# 55 payment-parameter boundary cases.
for i in range(55):
    mode = i % 9
    if mode == 0:
        ...
        p = {"amount": 5000, "currency": "usd"}
    else:
        p = {"amount": 5000.5, "currency": "USD"}
    add("parameter_constraints", "finance-agent", "payments", "create", p)

# 45 identity-violation combinations: the request itself is otherwise valid.
identity_templates = [
    ("hr-agent", "payments", "create", {"amount": 100, "currency": "USD"}),
    ("finance-agent", "files", "read", {"path": "/hr-docs/a.txt"}),
    ("unknown-agent", "payments", "create", {"amount": 2500, "currency": "EUR"}),
    ("unknown-agent", "files", "read", {"path": "/hr-docs/forms/a.pdf"}),
    ("finance-agent", "tickets", "read", {}),
]
for i in range(45):
    agent, tool, action, p = identity_templates[i % len(identity_templates)]
    add("identity_violation", agent, tool, action, dict(p))

# 40 known-tool unauthorized actions.
for i in range(40):
    if i % 2 == 0:
        add("action_authorization", "finance-agent", "payments", "delete", {"amount": 100 + i, "currency": "USD"})
    else:
        add("action_authorization", "hr-agent", "files", "write", {"path": f"/hr-docs/write-{i}.txt"})

# 40 path edge cases: traversal, look-alike prefixes, relative paths, case changes.
path_cases = [
...
for i in range(40):
    add("path_constraints", "hr-agent", "files", "read", {"path": path_cases[(i * 7) % len(path_cases)]})

# 55 malformed/type cases.
for i in range(20):
    add("malformed", "finance-agent", "payments", "create", {"amount": [i + 1], "currency": "USD"})
for i in range(10):
    add("malformed", "finance-agent", "payments", "create", {"amount": i + 1, "currency": None})
for i in range(10):
    add("malformed", "finance-agent", "payments", "create", {})
for i in range(10):
    add("malformed", "hr-agent", "files", "read", {"path": i})
for i in range(5):
    add("malformed", "finance-agent", "payments", "create", {"amount": {"value": 100}, "currency": "USD"})

# 15 unauthorized tools.
for i, tool in enumerate(["shell", "database", "browser", "email", "calendar", "secrets", "admin", "network", "storage", "code", "search", "system", "queue", "vault", "unknown-tool"]):
...
for i in range(2):
    tid = f"V2-MISSING-{i}"
    add("stateful_sequence", "finance-agent", "payments", "refund", {"transaction_id": tid, "amount": 100, "currency": "USD"}, [])
for i in range(2):
    tid = f"V2-REPLAY-{i}"
    add("stateful_sequence", "finance-agent", "payments", "refund", {"transaction_id": tid, "amount": 100, "currency": "USD"}, [{"id": tid, "event": "payment_created"}, {"id": tid, "event": "payment_refunded"}])
...
add("stateful_sequence", "finance-agent", "payments", "refund", {"transaction_id": "V2-OTHER", "amount": 100, "currency": "USD"}, [{"id": "OTHER", "event": "payment_created"}])
add("stateful_sequence", "finance-agent", "payments", "refund", {"transaction_id": "V2-UNKNOWN", "amount": 100, "currency": "USD"}, [{"id": "V2-UNKNOWN", "event": "payment_verified"}])
