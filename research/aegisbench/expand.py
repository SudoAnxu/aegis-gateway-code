...
def mutate(seed):
    ...
    def add(name, params):
        ...
        x["generator_version"] = "aegisbench-expand-v1"
        x["expected"], x["reason"] = decide(x)
        cases.append(x)

    # Missing/null/type mutations for every parameterized request.
    ...
    if "path" in base and isinstance(base["path"], str):
        p = base["path"]
        for value, name in [(p + "/../secret.txt","traversal"),("/hr-documents/foo.txt","prefix-collision"),("/hr-docs2/foo.txt","prefix-collision-2"),("/finance/reports/q2.txt","outside"),("/hr-docs/./foo.txt","dot-normalization")]:
            add(f"path-{name}", {**base, "path": value})

    # Irrelevant fields should not grant authority.
...
def main():
    ...
    # Stable deduplication by canonical scenario content.
    unique = {}
    ...
    payload = {
        ...
        "generator_version": "aegisbench-expand-v1",
